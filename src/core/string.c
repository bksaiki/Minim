...
MinimObject *minim_builtin_make_string(MinimEnv *env, size_t argc, MinimObject **args)
{
    ...
    if (!minim_exact_nonneg_intp(args[0]))
        THROW(minim_argument_error("exact non-negative integer", "make-string", 0, args[0]));
    ...
    else
    {
        if (!MINIM_OBJ_CHARP(args[1]))
            THROW(minim_argument_error("character", "make-string", 1, args[1]));
...
MinimObject *minim_builtin_string(MinimEnv *env, size_t argc, MinimObject **args)
{
    ...
    for (size_t i = 0; i < argc; ++i)
    {
        if (!MINIM_OBJ_CHARP(args[i]))
            THROW(minim_argument_error("character", "string", i, args[i]));
    }
...
MinimObject *minim_builtin_string_length(MinimEnv *env, size_t argc, MinimObject **args)
{
    if (!MINIM_OBJ_STRINGP(args[0]))
        THROW(minim_argument_error("string", "string-length", 0, args[0]));
...
MinimObject *minim_builtin_string_ref(MinimEnv *env, size_t argc, MinimObject **args)
{
    ...
    if (!MINIM_OBJ_STRINGP(args[0]))
        THROW(minim_argument_error("string", "string-ref", 0, args[0]));

    if (!minim_exact_nonneg_intp(args[1]))
        THROW(minim_argument_error("exact non-negative integer", "string-ref", 1, args[1]));
    ...
    if (idx >= len)
        THROW(minim_argument_error("out of bounds", "string-ref", 1, args[1]));
...
MinimObject *minim_builtin_string_setb(MinimEnv *env, size_t argc, MinimObject **args)
{
    ...
    if (!MINIM_OBJ_STRINGP(args[0]))
        THROW(minim_argument_error("string", "string-set!", 0, args[0]));

    if (!minim_exact_nonneg_intp(args[1]))
        THROW(minim_argument_error("exact non-negative integer", "string-set!", 1, args[1]));

    if (!MINIM_OBJ_CHARP(args[2]))
        THROW(minim_argument_error("character", "string-set!", 2, args[2]));
...
MinimObject *minim_builtin_string_copy(MinimEnv *env, size_t argc, MinimObject **args)
{
    ...
    if (!MINIM_OBJ_STRINGP(args[0]))
        THROW(minim_argument_error("string", "string-copy", 0, args[0]));
...
MinimObject *minim_builtin_string_fillb(MinimEnv *env, size_t argc, MinimObject **args)
{
    ...
    if (!MINIM_OBJ_STRINGP(args[0]))
        THROW(minim_argument_error("string", "string-fill!", 0, args[0]));

    if (!MINIM_OBJ_CHARP(args[1]))
        THROW(minim_argument_error("character", "string-fill!", 1, args[1]));
...
MinimObject *minim_builtin_substring(MinimEnv *env, size_t argc, MinimObject **args)
{
    ...
    if (!MINIM_OBJ_STRINGP(args[0]))
        THROW(minim_argument_error("string", "substring", 0, args[0]));

    if (!minim_exact_nonneg_intp(args[1]))
        THROW(minim_argument_error("exact non-negative integer", "substring", 1, args[1]));
    ...
    else
    {
        if (!minim_exact_nonneg_intp(args[2]))
            THROW(minim_argument_error("exact non-negative integer", "substring", 2, args[2]));
...
{
    if (!MINIM_OBJ_STRINGP(args[0]))
        THROW(minim_argument_error("string", "string->symbol", 0, args[0]));
...
MinimObject *minim_builtin_symbol_to_string(MinimEnv *env, size_t argc, MinimObject **args)
{
    ...
    if (!MINIM_OBJ_SYMBOLP(args[0]))
        THROW(minim_argument_error("symbol", "symbol->string", 0, args[0]));
...
MinimObject *minim_builtin_format(MinimEnv *env, size_t argc, MinimObject **args)
{
    ...
    if (!MINIM_OBJ_STRINGP(args[0]))
        THROW(minim_argument_error("string", "format", 0, args[0]));
...
MinimObject *minim_builtin_printf(MinimEnv *env, size_t argc, MinimObject **args)
{
    ...
    if (!MINIM_OBJ_STRINGP(args[0]))
        THROW(minim_argument_error("string", "printf", 0, args[0]));
