...
MinimObject *minim_builtin_hash_keyp(MinimEnv *env, size_t argc, MinimObject **args)
{
    if (!MINIM_OBJ_HASHP(args[0]))
        THROW(minim_argument_error("hash table", "hash-key?", 0, args[0]));
...
MinimObject *minim_builtin_hash_ref(MinimEnv *env, size_t argc, MinimObject **args)
{
    ...
    if (!MINIM_OBJ_HASHP(args[0]))
        THROW(minim_argument_error("hash table", "hash-ref", 0, args[0]));
    ...
    if (!res)
    {
        ...
        THROW(minim_error("no value found for ~s", "hash-ref", bf->data));
    }
...
MinimObject *minim_builtin_hash_remove(MinimEnv *env, size_t argc, MinimObject **args)
{
    ...
    if (!MINIM_OBJ_HASHP(args[0]))
        THROW(minim_argument_error("hash table", "hash-remove", 0, args[0]));
...
MinimObject *minim_builtin_hash_set(MinimEnv *env, size_t argc, MinimObject **args)
{
    ...
    if (!MINIM_OBJ_HASHP(args[0]))
        THROW(minim_argument_error("hash table", "hash-set", 0, args[0]));
...
{
    if (!MINIM_OBJ_HASHP(args[0]))
        THROW(minim_argument_error("hash table", "hash->list", 0, args[0]));
