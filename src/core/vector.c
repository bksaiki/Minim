...
MinimObject *minim_builtin_make_vector(MinimEnv *env, size_t argc, MinimObject **args)
{
    ...
    if (!minim_exact_nonneg_intp(args[0]))
        THROW(minim_argument_error("exact non-negative integer", "make-vector", 0, args[0]));
...
MinimObject *minim_builtin_vector_length(MinimEnv *env, size_t argc, MinimObject **args)
{
    if (!MINIM_OBJ_VECTORP(args[0]))
        THROW(minim_argument_error("vector", "vector-length", 0, args[0]));
...
MinimObject *minim_builtin_vector_ref(MinimEnv *env, size_t argc, MinimObject **args)
{
    ...
    if (!MINIM_OBJ_VECTORP(args[0]))
        THROW(minim_argument_error("vector", "vector-ref", 0, args[0]));

    if (!minim_exact_nonneg_intp(args[1]))
        THROW(minim_argument_error("exact non-negative integer", "vector-ref", 1, args[1]));
...
MinimObject *minim_builtin_vector_setb(MinimEnv *env, size_t argc, MinimObject **args)
{
    ...
    if (!MINIM_OBJ_VECTORP(args[0]))
        THROW(minim_argument_error("vector", "vector-set!", 0, args[0]));

    if (!minim_exact_nonneg_intp(args[1]))
        THROW(minim_argument_error("exact non-negative integer", "vector-set!", 1, args[1]));
...
{
    if (!MINIM_OBJ_VECTORP(args[0]))
        THROW(minim_argument_error("vector", "vector->list", 0, args[0]));
...
MinimObject *minim_builtin_list_to_vector(MinimEnv *env, size_t argc, MinimObject **args)
{
    ...
    if (!minim_listp(args[0]))
        THROW(minim_argument_error("list", "list->vector", 0, args[0]));
...
{
    if (!MINIM_OBJ_VECTORP(args[0]))
        THROW(minim_argument_error("vector", "vector-fill!", 0, args[0]));
