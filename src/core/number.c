...
MinimObject *minim_builtin_zerop(MinimEnv *env, size_t argc, MinimObject **args)
{
    if (!MINIM_OBJ_NUMBERP(args[0]))
        THROW(minim_argument_error("number", "zero?", 0, args[0]));
...
{
    if (!MINIM_OBJ_NUMBERP(args[0]))
        THROW(minim_argument_error("number", "negative?", 0, args[0]));
...
{
    if (!MINIM_OBJ_NUMBERP(args[0]))
        THROW(minim_argument_error("number", "positive?", 0, args[0]));
...
MinimObject *minim_builtin_evenp(MinimEnv *env, size_t argc, MinimObject **args)
{
    if (!minim_integerp(args[0]))
        THROW(minim_argument_error("integer", "even?", 0, args[0]));
...
{
    if (!minim_integerp(args[0]))
        THROW(minim_argument_error("integer", "odd?", 0, args[0]));
...
{
    if (!MINIM_OBJ_NUMBERP(args[0]))
        THROW(minim_argument_error("number", "exact?", 0, args[0]));
...
{
    if (!MINIM_OBJ_NUMBERP(args[0]))
        THROW(minim_argument_error("number", "inexact?", 0, args[0]));
