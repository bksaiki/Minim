...
MinimObject *minim_builtin_char_eqp(MinimEnv *env, size_t argc, MinimObject **args)
{
    if (!MINIM_OBJ_CHARP(args[0]))
        THROW(minim_argument_error("character", "char=?", 0, args[0]));

    if (!MINIM_OBJ_CHARP(args[1]))
        THROW(minim_argument_error("character", "char=?", 1, args[1]));
...
{
    if (!MINIM_OBJ_CHARP(args[0]))
        THROW(minim_argument_error("character", "char-ci=?", 0, args[0]));

    if (!MINIM_OBJ_CHARP(args[1]))
        THROW(minim_argument_error("character", "char-ci=?", 1, args[1]));
...
{
    if (!MINIM_OBJ_CHARP(args[0]))
        THROW(minim_argument_error("character", "char-alphabetic?", 0, args[0]));
...
{
    if (!MINIM_OBJ_CHARP(args[0]))
        THROW(minim_argument_error("character", "char-numeric?", 0, args[0]));
...
{
    if (!MINIM_OBJ_CHARP(args[0]))
        THROW(minim_argument_error("character", "char-whitespace?", 0, args[0]));
...
{
    if (!MINIM_OBJ_CHARP(args[0]))
        THROW(minim_argument_error("character", "char-upper-case?", 0, args[0]));
...
{
    if (!MINIM_OBJ_CHARP(args[0]))
        THROW(minim_argument_error("character", "char-lower-case?", 0, args[0]));
...
MinimObject *minim_builtin_int_to_char(MinimEnv *env, size_t argc, MinimObject **args)
{
    ...
    if (!minim_exact_nonneg_intp(args[0]))
        THROW(minim_argument_error("valid character integer value", "integer->char", 0, args[0]));
...
{
    if (!MINIM_OBJ_CHARP(args[0]))
        THROW(minim_argument_error("character?", "char->integer", 0, args[0]));
...
{
    if (!MINIM_OBJ_CHARP(args[0]))
        THROW(minim_argument_error("character", "char-upcase", 0, args[0]));
...
{
    if (!MINIM_OBJ_CHARP(args[0]))
        THROW(minim_argument_error("character", "char-downcase", 0, args[0]));
